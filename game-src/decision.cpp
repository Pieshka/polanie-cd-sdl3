...
for(i=0;i<20;i++)
{
    if(mem.c->b[i].exist==1&&mem.c->b[i].hp&&mem.c->b[i].hp<mem.c->b[i].maxhp)
    {
        int xx,yy,k=0;
...
if(mem.faza==1) //budowa krow
{
    if(!mem.index)
    {
        ...
        for(i=0;i<19;i++)
        {
            if(mem.c->b[i].type==2)//obora
            {
                for(int k=0;k<3;k++)
                {
                    if(mem.c->b[i].m[k].type==0&&mem.c->b[i].m[k].exist==1)j++;
                }
            }
            
        }
        if((!j)&&mem.c->milk<500)mem.c->milk=500;
    }
    char typ;
    if(mem.index>19)
    {
    ...
    if(mem.c->milk<450)return;
    if((mem.c->b[mem.index].type==2)&&(mem.c->b[mem.index].food!=2))//tylko obore gdzie mniej niz 2 krowy biezemy pod uwage
    {
        typ=2;
        Cmd[1].co=0;
        Cmd[1].nrb=mem.index;
