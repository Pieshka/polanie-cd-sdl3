...
char EndMap=0;
char kolorK[5][5]={{RED,RED,RED,RED,RED},{GREEN,GREEN,GREEN+1,GREEN+2,GREEN+3},
               //          czerwony        zielony                                zolty                                       niebieski           szary
{YELLOW,YELLOW,YELLOW+1,YELLOW+2,YELLOW+3},{BLUE,BLUE,BLUE+1,BLUE+2,BLUE+3},{GRAY,GRAY,GRAY+1,GRAY+2,GRAY+3}};
int wsp1[25][4]={{272,68,311,97},{246,67,271,106},{211,67,245,100},{1,8,43,48},{46,1,82,56},
{93,2,131,52},{138,7,176,37},{185,10,222,41},{4,68,40,111},{50,66,97,110},{105,76,144,100},
{148,76,191,102 },{6,121,34,145},{41,119,65,146},{76,121,106,146},{118,117,153,147},
{163,116,202,145},{8,169,36,193},{43,151,84,197},{86,168,123,197},{129,161,164,197},
{223,112,261,147},{174,171,216,192},{219,162,260,192},{269,160,314,195}};
//--------------------------------------------------------------------------
int wsp2[25][2]={{110,10},{90,15},{63,23},{60,49},{86,24},{114,35},
{142,44},{171,49},{81,79},{102,73},{134,69},{160,76},
{94,114},{115,104},{135,89},{163,98},{188,89},{108,128},
{130,104},{156,123},{191,112},{212,103},{117,142},{153,145},{182,142}};
...
for(i=0;i<25;i++)
        {
        SetScreen(1);
        GetImage13h(wsp1[i][0],wsp1[i][1],wsp1[i][2]+1,wsp1[i][3]+1,kraina);
        SetScreen(0);
        PutImageChange13h(wsp2[i][0],wsp2[i][1],kraina,1,ACTIVE,kolorK[(prowintion[i]-1)][0]);
        }
...
do{
  ...
  PutImage13h(mouse.X,mouse.Y,buttons[6],1);
    do{
      if(licznik==4||licznik==16||licznik==8||licznik==12||licznik==20||licznik==24)
      {
      if(kraina!=NULL)
      sw1=licznik/4;
      if(sw1==5)sw1=3;
      ...
      for(i=0;i<25;i++)
        ...
        GetImage13h(wsp1[i][0],wsp1[i][1],wsp1[i][2]+1,wsp1[i][3]+1,kraina);
        SetScreen(0);
        if(mouse.oldX>wsp2[i][0]-16&&mouse.oldY>wsp2[i][1]-14&&mouse.oldX<wsp2[i][0]+40&&mouse.oldY<wsp2[i][1]+40)
           {
           PutImage13h(mouse.oldX,mouse.oldY,Mysz[0],0);
           }
        if(prowintionA[i])
            PutImageChange13h(wsp2[i][0],wsp2[i][1],kraina,1,ACTIVE,kolorK[(prowintion[i]-1)][sw1]);

        if(mouse.oldX>wsp2[i][0]-16&&mouse.oldY>wsp2[i][1]-14&&mouse.oldX<wsp2[i][0]+40&&mouse.oldY<wsp2[i][1]+40)
          {
          GetImage13h(mouse.oldX,mouse.oldY,mouse.oldX+15,mouse.oldY+14,Mysz[0]);
          PutImage13h(mouse.oldX,mouse.oldY,buttons[6],1);
          }
    }
     }
     if(licznik>23){licznik=0;}
...
switch(prowintion[level-1])
  {
  case 5:CenterText13h(10,70,270,90,"Atakujemy Wislan",1);
         CenterText13h(11,70,271,90,"Atakujemy Wislan",GRAY+1);break;    //szary
  case 3:CenterText13h(10,70,270,90,"Atakujemy Wieletow",1);
         CenterText13h(11,70,271,90,"Atakujemy Wieletow",YELLOW+1);break;     //zolty
  case 4:CenterText13h(10,70,270,90,"Atakujemy Mazurow",1);
         CenterText13h(11,70,271,90,"Atakujemy Mazurow",BLUE+1);break;    //niebieski
