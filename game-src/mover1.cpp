...
/*------------dane o postaciach-------------------------------------------------
                    ...
                    | | | |  |  |-----maxdelay
                    | | | |  |  |   |-???*/
int Udata[13][7]={{2,0, 0,0,100,12,  0},//krowa
...
int dmagic[15] ={60,80,85,90,120,140,150,160,170,180,190,200,220,240,280};
...
int Who(int nr)//0 nikt 1-nasz wojownik 2-nasz budynek 3-ich wojownik 4-ich budynek
{

    int aa=nr&0x00ff;
    int bb=nr>>8;
    if(bb<1||bb>2)return 0;
    bb--;
    int nrb=aa/10; //nr budynku
    int nrp=aa-nrb*10;// nr postaci
    if(nrp>3||nrb>19) //jeden z zolnierzy
    {
        if(bb)return 3;//ich zolnierz
        else return 1;//nasz zolnierz
    }
    if(bb)return 4;//ich budynek
    else return 2;//nasz budynek

}
int Type(int nr)//0 nikt 1-nasz wojownik 3-nasz budynek 2-ich wojownik 4-ich budynek
{
...
Mover1* Pointer(int nr)
{
    ...
    if(nrp>3||nrb>19) //jeden z zolnierzy
    {
        ...
        {
           // char ss[20];
           // sprintf(ss,"Pointer c:%d, b:%d, p:%d, u:%d",bb,nrb,nrp-4,castle[bb].b[nrb-1].m[nrp-4].udder);
           // strcpy(Msg.msg,ss);
           // Msg.licznik=20;
...
void NewCow(int nr)
{
    ...
    if(oldcow==NULL)return;
    //oldcow->wybrany=1;
    int side=1;
    if(nr>512&&nr<768)side=0;
    ...
    for(int j=0;j<6;j++)
    {
        ...
        if(!(castle[side].b[i].m[j].exist))
        {
           ...
           // Msg.licznik=200;
            ...
            place[oldcow->x][oldcow->y]=0;
            oldcow->exist=0;
            castle[side].b[i].m[j].Init(oldcow->type,oldcow->x,oldcow->y,1,10);
...
do
{
 ...
 if(stopk!=startk)
 {
    i=kolejka[stopk];
    j=kolejka[stopk+1];
        if(stopk<1950)stopk+=2;else stopk=0;
 }
      ...
      {if(Place[ii][jj]==1001)
    {
      ...
      if(Place[ii][jj]==1000)
       if(ii>0&&ii<MaxX-1&&jj>0&&jj<MaxY-1)
             ...
             kolejka[startk]=ii;
             kolejka[startk+1]=jj;
             if(startk<1950)startk+=2;else startk=0;
             }
       }
...
for(i=0;i<MaxX;i++)
{
    Place[i][j]=1;
    if(!place[i][j])Place[i][j]=1000;
    if(place[i][j]>255&&place[i][j]<500&&!(*distance))Place[i][j]=1001;
    if(place[i][j]>511&&place[i][j]<512+255&&(*distance==1))Place[i][j]=1001;
    if((place[i][j]>drzewa0)&&((*distance)==2))Place[i][j]=1001;
}
...
for(i=0;i<MaxX;i++)
{
    ...
    if((i>0)&&(j>0)&&(i<MaxX-1)&&(j<MaxY-1))
    {
        if(!(place[i][j]))Place[i][j]=1000;//wolne
        if(placeG[i][j]==256)Place[i][j]=1234;
    }
...
switch(t)
{
       ...
       case 5:                   //miecz
       case 1:view=NULL;break;   //topor

       case 2:exist++;view=missiles[0][dx+1][dy+1];break;  //luk
       case 12:exist++;view=missiles[5][dx+1][dy+1];break;  //kusznik
       case 3:exist+=3;view=missiles[1][dx+1][dy+1];break;  //grom
       case 13:exist+=3;view=missiles[1][dx+1][dy+1];break;  //grom
       case 11:exist+=3;view=missiles[4][dx+1][dy+1];break;  //magowy
       case 4:exist+=3;view=missiles[2][dx+1][dy+1];break;  //duzy ogien
       case 14:exist+=3;view=missiles[2][dx+1][dy+1];break;  //duzy ogien
       case 6:exist+=2;view=missiles[3][dx+1][dy+1];break;  //wlocznia
}
...
void Missile::Move()
{
    ...
    if(exist==2)  //dzwiek wystrzalu
    {
        ...
        switch (type)
        {
            ...
            case 4:
            case 14:kk=5;break;
            case 11:kk=6;break;//duch
            case 12:kk=7;break;//kusza
        }
        if(Msg.dzwiek<kk){Msg.dzwiek=kk;Msg.X=xt;Msg.Y=yt;}
    }
    if(exist>1){exist--;return;}

    if(visible)visible--;
    if(visible==100){exist=0;return;}
    if(visible>100)
    {
        attack[xt][yt]+=damage;
        return;
    ...
    else
    {
        ...
        if(type==4||type==11||type==12){exist=130;}
        attack[xt][yt]+=damage;
        //gromobicie
        if(type==13)
        {
        ...
        if(type==14)
        {
            ...
            attack[xt+1][yt]+=damage;
            attack[xt-1][yt]+=damage;
            if((placeN[xt][yt]<50||placeG[xt][yt]<112)&&(placeG[xt][yt]<74||placeG[xt][yt]>86))
            {
                placeN[xt][yt]=100;
            }
            if((placeN[xt+1][yt]<50||placeG[xt+1][yt]<112)&&(placeG[xt+1][yt]<74||placeG[xt+1][yt]>86))
            {
                placeN[xt+1][yt]=100;
            }
            if((placeN[xt-1][yt]<50||placeG[xt-1][yt]<112)&&(placeG[xt-1][yt]<74||placeG[xt-1][yt]>86))
        ...
        /////kaplan i ognie
        if(type==4&&(placeN[xt][yt]<50||placeG[xt][yt]<112)&&(placeG[xt][yt]<74||placeG[xt][yt]>86))
        {
            placeN[xt][yt]=100;
        }
        if(type==4)
        {//rozrzut ognia po strzale
            if(dx>-3&&dx<3)dx=0;
            if(dx>2)dx=1;
            if(dx<-2)dx=-1;
            if(dy>-3&&dy<3)dy=0;
            ...
            if(dy<-2)dy=-1;
            xt+=dx;yt+=dy;
            if(placeG[xt][yt]<74||placeG[xt][yt]>86){placeN[xt][yt]=90;attack[xt][yt]+=(damage-10);}
            xt+=dx;yt+=dy;
            if(placeG[xt][yt]<74||placeG[xt][yt]>86){placeN[xt][yt]=80;attack[xt][yt]+=(damage-20);}
        }
            ...
            if(!place[xt][yt])
            {
                ...
                if(tar!=NULL)
                {
                    if(SDL_abs(xt-tar->x)<2&&SDL_abs(yt-tar->y)<2) // [PORT] Replace abs with SDL_abs
                    {
                        xt=tar->x;
                        yt=tar->y;
                        if(diff)attack[xt][yt]+=(damage>>1);
                        else attack[xt][yt]+=(damage>>2);
                    }
                }
            }
        }
        //dzwieki uderzen
        if(type==8&&Msg.dzwiek<8){Msg.dzwiek=8;Msg.X=xt;Msg.Y=yt;}
        if(type==9&&Msg.dzwiek<9){Msg.dzwiek=9;Msg.X=xt;Msg.Y=yt;}
        if(type==7||type==5||type==1)
        {
            if(place[xt][yt]>768)dx=1;else dx=2;
            if(Msg.dzwiek<dx){Msg.dzwiek=dx;Msg.X=xt;Msg.Y=yt;}
        }
        //duch
        if(type==11)
        {
    ...
    if(view!=NULL)
    {
        ...
        if(a>10&&a<=259&&b>7&&b<185)
        {
            if(visible>100)//duszacy duszek
            {
                ...
                {
                    if(dx>0)PutImageChangeInverse13h(a,b,view,1,1,1);
                    else PutImage13h(a,b,view,1);
...
{
magic=0;
if(t==3||t==4||t==11)magic=dmagic[0]/2;
wybrany=0;
exp=0;
type=t;
...
{
IFF=Nr;
if(IFF==2&&diff>1){hp+=15;maxhp+=15;}
if(IFF==2&&!diff){hp-=25;maxhp-=25;}
}
//---------------------------------
void Mover1::SetCommand(int Nr)
{
if(type!=1&&(Nr==8||Nr==11))return;
if(Nr==8)
{
    xm=xe;
    ym=ye;
...
if(Nr==2)ispath=0;
if(Nr==5)mainTarget=0;
if(type&&Nr==123&&(x&1))return;
commandN=Nr;
}
//----------------------------------
//----------------------------------
...
for(i=0;i<MaxX;i++)
    ...
    {
        Place[i][j]=1000;
        if(placeN[i][j]>10&&placeN[i][j]<100)Place[i][j]=9999;
    }
    else Place[i][j]=9999;
...
if(Place[x][y]==1000)  //1 ruch w kierunku
    {
        if(!type)return;
        int ddx,ddy;
        ddx=x-xe;if(ddx<0)ddx=-1;if(ddx>0)ddx=1;
        ddy=y-ye;if(ddy<0)ddy=-1;if(ddy>0)ddy=1;
        path[0][0]=ddx;
        path[0][1]=ddy;
        ispath=1;
...
Place[i][j]=1000;
min=1000;
if(Place[i-1][j]<min){min=Place[i-1][j];mini=i-1;minj=j;}
if(Place[i+1][j]<min){min=Place[i+1][j];mini=i+1;minj=j;}
if(Place[i][j-1]<min){min=Place[i][j-1];mini=i;minj=j-1;}
if(Place[i][j+1]<min){min=Place[i][j+1];mini=i;minj=j+1;}
if(Place[i-1][j-1]<min){min=Place[i-1][j-1];mini=i-1;minj=j-1;}
if(Place[i-1][j+1]<min){min=Place[i-1][j+1];mini=i-1;minj=j+1;}
if(Place[i+1][j-1]<min){min=Place[i+1][j-1];mini=i+1;minj=j-1;}
if(Place[i+1][j+1]<min){min=Place[i+1][j+1];mini=i+1;minj=j+1;}
...
Path[0][1]=j-minj;
ispath=1;
if(min==0){path[0][0]=Path[0][0];path[0][1]=Path[0][1];return;}
do
{
i=mini;
j=minj;
...
{
    commandN=0;
    if(!type&&command==3){command=1;commandN=1;delay=0;}
    return;
}
...
{
     if((type)&&(x-xe<2)&&(x-xe>-2)&&(y-ye<2)&&(y-ye>-2))
     {
        commandN=0;
     }
     if(!type)
...
if(IFF==2&&hp>(maxhp-20))//komputerowy
{
    ...
    for(j=y-k+1;j<=y+k-1&&j<MaxY-1;j++)
    {
        ...
        {
            case 1:NoOfEnemies++;break;
            case 3:NoOfOurs++;break;
        }
    }
    ...
    for(j=y-k;j<=y+k&&j<MaxY-1;j++)
    {
        if(i>0&&i<MaxX-1&&j>0&&j<MaxY-1)
        {
            if(place[i][j]&&place[i][j]!=nr)
            {
                d=place[i][j];
                int dd=Who(d);
                if(((d&0xff00)!=iff)&&(dd==1||dd==3))
                {//postac
                        target=d;
                        if(type!=10)return 1;
                        Mover1 *mm=Pointer(target);
                        if((!mm->type)||(mm->type==9))return 1;
                }
            }
        }
    }
}
if(!d||IFF==1)return 0; //nasi nie atakuja budynkow
if(type==11)return 0;//Mag tez nie
for(k=1;k<=s_range+drange;k++)
{
    for(i=x-k;i<=x+k&&i<MaxX-1;i++)
    for(j=y-k+1;j<=y+k-1&&j<MaxY-1;j++)
...
int i,j;
if(!target)return 0;
if(place[xe][ye]==target)return 1;
for(i=1;i<MaxX-1;i++)
for(j=1;j<MaxY-1;j++)
{
    if(place[i][j]==target)
...
if(maxx<0)maxx=-maxx;
if(maxy<0)maxy=-maxy;
if(maxy<maxx)return maxx;else return maxy;
}
//------------------------------------------------
void Mover1::Attack()
{
inmove=0;
if(target<768)ispath=0;
if(target>768&&type!=1&&type!=4){target=0;mainTarget=0;}
if(!target){phase=0;delay=maxdelay;return;}
...
if(Distance()<=a_range)
{
        if(type==10)
        {
            ...
            if(oldcow!=NULL)oldcow->SetCommand(123);//zmiana
            dx=x-xe;
            dy=y-ye;
            if(dx>1)dx=1;if(dx<0)dx=-1;if(dy>1)dy=1;if(dy<0)dy=-1;
            delay=10;
            target=0;
            commandN=0;
            return;
        ...
        if(IFF==2&&type==11&&magic<11){SetEnd(x+dx*3,y+dy*3);Move();return;}
        if(IFF==2&&type==4&&magic<21){SetEnd(x+dx*3,y+dy*3);Move();return;}//ucieczka
        delay=maxdelay;

        if(type==3&&magic<11){phase=0;dx=0;dy=0;return;}
        if(type==4&&magic<21){phase=0;dx=0;dy=0;return;}
        if(type==11&&magic<21){phase=0;dx=0;dy=0;return;}
        ...
        if(type==4)magic-=20;
        if(type==11)magic-=20;
        if(IFF==2&&place[xe][ye]>800)damage+=20;
        ...
        if(target<768&&exp<(15*16)-5)
        {
            ...
            {
                exp++;
                if(type==3||type==4||type==11)exp+=3;
            }

        }
        inattack=1;
        if(dx>1)dx=1;if(dx<0)dx=-1;if(dy>1)dy=1;if(dy<0)dy=-1;
        if(!placeN[x][y])placeN[x][y]=1;
        if(!placeN[xe][ye])placeN[xe][ye]=1;
...
inmove=0;
inattack=0;
if(placeN[xe][ye]>226||placeG[xe][ye]==47||(placeG[xe][ye]>265&&placeG[xe][ye]<277)){commandN=0;placeN[xe][ye]=1;delay=maxdelay;return;}
if(Distance()==1)
    {
        ispath=0;
        if(placeG[xe][ye]==277&&place[xe][ye]){delay=maxdelay;return;}
        dx=x-xe;
        dy=y-ye;
        delay=maxdelay;
        inattack=1;
        ...
        if(Msg.dzwiek<2){Msg.dzwiek=1;Msg.X=x;Msg.Y=y;}
    return;
    }
xe=xm;ye=ym;
Move();
...
void Mover1::Run()
{
    if(type)Run1();else Run2();
}
//////////////////////////////////////////////////
void Mover1::Run1()    // rycerze
{
    if(!exist)return;
    if(exist==2&&delay<1){exist=0;return;}
    if(exist==2){delay--;return;}   // trup

    if(attack[x][y])
    {
        int dem;
        if(marmour)attack[x][y]=0;
        if(attack[x][y]>0)
        {
            dem=(int)attack[x][y]-armour-darmour[exp>>4];
            attack[x][y]=0;
            if(dem<=0)dem=1;
            if(!mainTarget)if(command==2)commandN=0;
            if((command==1||command==8)&&type!=10&&placeG[xe][ye]!=256&&(hp>(maxhp>>1)))commandN=0;  // zatrzymaj idacych!
    ...
    if(delay)return;      // wyjdz jezeli nie koniec ruchu
    if(drange)drange--;
 ...
 /*   if(hp<5+(diff>>4)&&IFF==2&&!drange&&command!=1)//&&diff
    ...
    if(hp==maxhp&&IFF==2&&!command&&placeG[x][y]==256)
    {// wyleczony odsun sie z miejsca leczenia
        SetEnd(x+5,y+5);
        SetCommand(1);
    }
    inattack=0;
    ...
    if(placeG[x][y]==256&&hp<maxhp)
    {
        ...
        if(type==3||type==4||type==9||type==11)magic+=5;
    }
    if(type==3||type==4||type==9||type==11)
    {
        magic++;
        if(type==11)magic++;
        if(magic>dmagic[exp>>4])magic=dmagic[exp>>4];
        if(type==9&&magic>9&&hp<maxhp)
        {//autoleczenie strzygi
            hp+=10;
            magic-=10;
        }
    }
    ...
    if(command==10)//teleport
    {
        commandN=0;
        Hide();
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)xe+=3;//=0
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)ye+=3;//1
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)xe-=3;//2
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)xe-=3;//3
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)ye-=3;//4
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)ye-=3;//5
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)xe+=3;//6
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2)xe+=3;//7
        if(place[xe][ye]||xe<1||ye<1||xe>MaxX-2||ye>MaxX-2){xe=x;ye=y;}
        x=xe;
        y=ye;
        Show();
        inattack=1;
    ...
    switch(command)
    {
        case 123:NewCow(nr);return;
        case  0:if(LookAround())
                {
                    commandN=2;
                    return;
        ...
        case  7:
                ...
                {
                    inattack=Distance();
                    if(inattack>9){inattack=0;Move();return;}
                    if(missile.exist)return;
                    inmove=0;
                    inattack=1;
                    dx=0;dy=0;
                    int Mtype=14;
        ...
        case  9:
                ...
                {                 //widzenie
                    ...
                    magic-=25;
                    marmour=5;
                    if(Msg.dzwiek<12){Msg.dzwiek=11;Msg.X=x;Msg.Y=y;}
                    int xx,yy,i,j;
                    for(i=1;i<MaxX;i++)
                    for(j=1;j<MaxY;j++)
                    {
          ...
          case 11://rabanie palisady
                if(type!=1&&type!=4){command=0;commandN=0;return;}  
                target=2;
                FindEnemy(x,y,&xe,&ye,&target);
                target=place[xe][ye];
                if(xe==32&&ye==32)target=0;
                if(target==nr)target=0;
...
void FindSheed(int k,int x,int y,int*xm,int*ym)
{
  int i;
        ...
        for(i=0;i<20;i++)
        {
            if(!place[castle[k].b[i].x][castle[k].b[i].y+1]&&castle[k].b[i].type==2&&castle[k].b[i].exist==1)
            {
                *xm=castle[k].b[i].x;
                *ym=castle[k].b[i].y+2;
            }
...
if(attack[x][y])
{
        ...
        attack[x][y]=0;
        if(dem>0)hp-=dem;
        if(dem>4)ShowHit=4;else ShowHit=3;
        if(hp<=0)
        {
                exist=2;
                dx=0;dy=0;
...
inattack=0;
phase=0;
if(placeG[x][y]==256&&hp<maxhp)hp++;   //leczenie w swietym miejscu
command=commandN;
...
if(command==123){NewCow(nr);return;}
if(command==1){Move();return;}
if(command==2){Graze();return;}  //jezeli nic nie robi to rozejzec sie
if(command==3&&x==xm&&y==ym){commandN=4;inmove=0;return;}//oddawaj
if(command==3){Move();return;}    //wracaj
if(command==4)return;
if(command==5)
{
    //ucieczka do obory
...
if(x0==0)x0=1;
if(y0==0)y0=1;
if(y0>MaxY-2)y0=MaxY-2;
if(x0>MaxX-2)x0=MaxX-2;

if(!type)
{
    if((placeG[x0][y0]==163&&place[x0][y0-1]<512)||(place[x0][y0]<512&&placeG[x0][y0]>157&&placeG[x0][y0]<166))
    {
...
else
{
   if(inattack)
   {
   if(delay>maxdelay-2){phase=3;}
   if(delay==Phase[type][17]){phase=4;}
   if(delay==Phase[type][18]){phase=1;if(!type)phase=0;}
...
if(x<(x0-1)||y<(y0-1)||x>(x0+17)||y>(y0+14))visible=0;  //ze wzgledu na LookUpTable
    else visible=1;
if(visible)
{
    if(inmove==1)
    {
        if(dx>0)xr=Xe[x-x0+1]+(int)shiftX[type][delay]-16;
        if(dx<0)xr=Xe[x-x0+1]-(int)shiftX[type][delay]-16;
        if(!dx)xr=Xe[x-x0+1]-16;
        if(dy>0)yr=Ye[y-y0+1]+(int)shiftY[type][delay]-14;
        if(dy<0)yr=Ye[y-y0+1]-(int)shiftY[type][delay]-14;
        if(!dy)yr=Ye[y-y0+1]-14;
    }
    else
    {
        xr=Xe[x-x0+1]-16;
    ...
    if(inattack&&phase>2&&(type==1||type==5||(type>6&&type<10)))
    {
        if(dy>0)yr-=7;
        if(dx>0)xr-=8;
    }
...
void Mover1::ShowS()
{int k;
if(!exist||!visible||!placeN[x][y])return;

if(exist==2)
{
    k=2;
    if(delay<200)k=1;
    if(delay<70)k=0;    // k-faza rozkladu typ-typ postaci
    if(k)
    {
        if(IFF==1)PutImage13h(xr,yr,movers[5-k][type][1][1],1);
        else PutImageChange13h(xr,yr,movers[5-k][type][1][1],1,color1,color2);
    }
    else
    {
        k=0;
        if(type==8)k=2;
        if(!type)k=1;
        if(IFF==1)PutImage13h(xr,yr,dead[k],1);
        else PutImageChange13h(xr,yr,dead[k],1,color1,color2);
    }
    return;
}
if(!dx&&!dy)dy=1;
...
{
    int a=0,b=0;
    if(inattack&&phase>2&&(type==1||type==5||type==7||type==8||type==9))
    {
        if(dy>0)b=7;
        if(dx>0)a=8;
    }
    if(IFF==1)PutImageChange13h(xr+a,yr+b,Mysz[5],1,color1,ExpColor[exp>>4]);
    else PutImageChange13h(xr+a,yr+b,Mysz[5],1,color1,ExpColor[(exp>>4)+15]);
    int color=LightGreen;
    if(hp<(maxhp>>1))color=Yellow;
    if(hp<(maxhp>>2))color=LightRed;
    ...
    if(magic&&IFF==1)
    {
        ...
        Bar13h(xr+a+16,yr+b+14-hh,xr+a+17,yr+b+14,LightBlue);
    }
    if(!type&&IFF==1)
    {
        hh=((udder*14)/100);
        ...
        if(hh)Bar13h(xr+a+16,yr+b+14-hh,xr+a+17,yr+b+14,White);
...
if(dx<0)
    ...
    if(movers[phase][type][0][1-dy]!=NULL)
            {
             if(IFF==1)PutImageChangeInverse13h(xr,yr,movers[phase][type][0][1-dy],1,color1,color1);
                 else PutImageChangeInverse13h(xr,yr,movers[phase][type][0][1-dy],1,color1,color2);
            }
    if(marmour)PutImage13h(xr,yr,picture[278+(marmour&3)],1);
    if(placeG[x][y]==256&&hp<maxhp)PutImageChange13h(xr,yr,picture[282+(delay&1)],1,Red,LightBlue);
    }
else
   {
   if(movers[phase][type][1-dx][1-dy]!=NULL)
           {
            if(IFF==1)PutImage13h(xr,yr,movers[phase][type][1-dx][1-dy],1);
                else PutImageChange13h(xr,yr,movers[phase][type][1-dx][1-dy],1,color1,color2);
           }
   if(marmour)PutImage13h(xr,yr,picture[278+(marmour&3)],1);
   if(placeG[x][y]==256&&hp<maxhp)PutImageChange13h(xr,yr,picture[282+(delay&1)],1,Red,LightBlue);
   }

if(ShowHit>2)PutImage13h(xr,yr,Hit[1],1);
else if(ShowHit>0)PutImage13h(xr,yr,Hit[0],1);
}
//-----------------------------------------------------------------------------
void Mover1::Show()
{
    ...
    if(IFF==1)
    {
        for(i=-s_range;i<=s_range;i++)
        {
            ...
            for(j=-s_range;j<=s_range;j++)
            {
                ...
                {
                    if((i!=-s_range||j!=-s_range)&&(i!=-s_range||j!=s_range)&&(i!=s_range||j!=-s_range)&&(i!=s_range||j!=s_range))//bez rogow
                    {
                        placeN[x+i][y+j]|=1;
                    }
                }
...
switch(type)
    {
        case 1:maxfood=0;
                maxhp=400;break;
        case 6:maxfood=1;maxhp=400;break;
        case 2:
        case 4:maxfood=3;maxhp=350;break;
        case 3:maxfood=6;maxhp=350;break;
        case 5:maxfood=4;maxhp=350;break;
        }
x=x0-2;
y=y0-2;

armour=10;
if(iff==1)maxhp=maxhp>>1;
hp=30;
IFF=iff;
...
nr=iff*256+Nr*10;
faza=0;
for(i=0;i<6;i++){m[i].exist=0;m[i].missile.exist=0;}
for(j=0;j<3;j++)
for(i=0;i<3;i++)
         {
...
if(IFF==1)
{
    ...
    for(i=-2;i<5;i++)
    {
         ...
         if(!placeN[x+i][y+j])placeN[x+i][y+j]=1;
    }
}
place[x][y]         = nr;
place[x+1][y]       = nr;
...
if(exist==3&&hp>=maxhp)//koniec budowy
    {
        ...
        place[x+1][y+1]=nr;
        if(Msg.dzwiek<18){Msg.dzwiek=18;Msg.X=x;Msg.Y=y;}
        }
...
if(exist==2&&!food){exist=0;return;}  //jezeli zburzony i brak postaci to zniknij
if(exist!=1)food=maxfood;
//--- sprawdzenie atakow
dd=0;
if(exist==1||exist>2)
{
 attack[x+1][y+1]=0;
 for(i=x;i<x+3;i++)
 for(j=y;j<y+3;j++)if((i!=x)||j<2+y){dd+=attack[i][j];attack[i][j]=0;}

 if(dd>armour)dd-=armour;
 else dd=0;
}
...
if(dd)
    {
        ...
        if(IFF==2)
        {
            ...
            {
                if(castle[1].m[post].type&&castle[1].m[post].type<10)castle[1].m[post].drange=7;
            }
            for(int build=0;build<20;build++)
            {
                for(int post=0;post<6;post++)
                {
                    if(castle[1].b[build].m[post].type)castle[1].b[build].m[post].drange=7;
                }
            }
        }
        hp-=dd;
        if(hp<=0)
            {
                ...
                placeN[x][y+2]=79;
                exist=2;
                if(Msg.dzwiek<17){Msg.dzwiek=17;Msg.X=x;Msg.Y=y;}
        }
        }

if(exist!=1)return;
if(hp<maxhp)placeN[x+1][y+1]=1;
if(hp<maxhp/2)placeN[x+1][y+1]=74;
if(hp<maxhp/4)placeN[x+1][y+1]=78;
if(hp<maxhp/8)placeN[x+1][y+1]=100;
faza++;
if(faza>4)faza=0;
//flaga
if(exist==1&&faza==1)
    switch(type)
        {
        case 1:placeG[x+1][y]=137+9;break;//?????
                case 2:placeG[x+2][y+1]=157+9;break;//obora
                case 3:placeG[x+2][y]=177+9;break;//koszary1
                case 4:placeG[x+1][y+1]=197+9;break;// swiatynia
                case 5:placeG[x+1][y+1]=217+9;break;// koszary1
                case 6:placeG[x+2][y]=237+9;break;// bohater
                }
if(exist==1&&faza==3)
    switch(type)
        {
        case 1:placeG[x+1][y]=137+1;break;//?????
                case 2:placeG[x+2][y+1]=157+5;break;//obora
                case 3:placeG[x+2][y]=177+2;break;//koszary1
                case 4:placeG[x+1][y+1]=197+4;break;// swiatynia
                case 5:placeG[x+1][y+1]=217+4;break;// koszry1
                case 6:placeG[x+2][y]=237+2;break;// bohater
                }
...
{
int i;
for(i=0;i<6;i++)m[i].Prepare(X,Y,typ);
}
...
int Building::Wynik(int r)
{
    if(type==1)
    {
        if(r>2)return r+1;//obora,koszary 1,swiatynia
        if(r==1)return 2; //droga
            return 1;
    }
    if(type==2&&r==2)//krowa
        return 0;
    ...
    if(type==3)
    {
        if(r==5)return 7;        //koszary 2
        if(r==2||r==3)return 0;  //topor,luk
        return 1;                //palisada
    }
    if(type==4)
    {
        if(r==5||r==3||r==6)return 0;  //druidka,druid,mag
    }
    if(type==5)
    {
        if(r==5)return 8;   // dom wodza
        if(r==4||r==3)return 0;//miecz,wlocznia
        if(r==1)return 3;   //palisada
        return 1;           //odbudowa
    }
    if(type==6&&(r==5||r==6))
        return 0;
...
int i;
if(type!=2)return 0;
i=m[0].Milk()+m[1].Milk()+m[2].Milk()+m[3].Milk()+m[4].Milk()+m[5].Milk();
return i;
}
////////////////////////////////////////////////////////////////////////
...
if(exist!=1)return 0;
j=20;
if(food>=maxfood)return 0;
for(i=5;i>=0;i--)if(!m[i].exist)j=i; //znajdz wolny
//if(place[x][y+2]!=0)return 0;  // nie mozna utorzyc goscia
i=20;
if(type==2&&Nr==2){i=0;p=450;}//krowa
if(type==2&&Nr==6&&level>PASTUCH_LEV){i=10;p=1250;}//pastuch
if(type==3&&Nr==2){i=1;p=450;}//topor
if(type==3&&Nr==3){i=2;p=650;}//luk
if(type==4&&Nr==3){i=3;p=650;}//druidka   //i=typ
if(type==4&&Nr==5){i=4;p=1050;}//kaplan
if(type==4&&Nr==6&&level>MAG_LEV){i=11;p=1250;}//mag
if(type==5&&Nr==3){i=5;p=650;}//miecz
if(type==5&&Nr==4){i=6;p=850;}//wlocznia
if(type==6&&Nr==5){i=7;p=1050;}//dowodca
if(type==6&&Nr==6&&level>KUSZNIK_LEV){i=12;p=1250;}//kusznik
if(i==20)return 0; // zla kolejnosc nie mozna zbudowac takiego goscia w tym budynku
int xx=0,yy=0;
if(place[x][y+2])
{
    for(int i1=x-1;i1<x+4;i1++)
    for(int j1=y-1;j1<y+4;j1++)if(!place[i1][j1]){xx=i1;yy=j1;}
}
else
{
    xx=x;
...
milk=mlk;
maxmilk=mlk;
for(i=0;i<MaxUnitsInCastle;i++)m[i].Disable();
for(j=0;j<20;j++)
{
    b[j].exist=0;
    for(i=0;i<6;i++)b[j].m[i].Disable();
}
...
for(i=0;i<20;i++)b[i].ShowS(Xs,Ys,what);
...
if(command.co==2)//komendy ogolne
    {
        if(command.command==2&&milk>=45&&!place[command.x][command.y])//droga
          {
          ...
          Droga(command.x,command.y+1,35);
          milk-=45;
          if(Msg.dzwiek<20){Msg.dzwiek=20;Msg.X=command.x;Msg.Y=command.y;}
          }
        if(command.command==2&&milk>=45&&place[command.x][command.y]==1)//most
          {
        ...
        if(command.command>3)//budowa budynku
          ...
          int w;
          w=20;
          for(i=0;i<20;i++)if(!b[i].exist)w=i;
          i=command.command-1;
          if(i>5)i=5;
          if(milk<i*200+50)return;
          if(w<20)
              {
              b[w].Init(command.x,command.y,command.command-2,IFF,w);
              milk-=i*200+50;
              if(Msg.dzwiek<20){Msg.dzwiek=20;Msg.X=command.x;Msg.Y=command.y;}
              command.y-=2;command.x-=2;
              for(int ii=-1;ii<=3;ii++)
              for(int jj=-1;jj<=3;jj++)Droga(command.x+ii,command.y+jj,35);
              }
          }
        }
...
if(command.co==1)//komendy dla modulow
    {
        ...
        if(command.nrb<20)
                ...
                else mm=&m[command.nrm];

        mm->SetCommand(command.command);
        if(command.command==1||command.command==7||command.command==8)mm->SetEnd(command.x,command.y);
        if(command.command==2)mm->SetTarget(place[command.x][command.y]);
        }
class Mover1* mm;
if(command.co==3)//komendy dla modulow wszystkich
    {
        ...
        if(mm->wybrany)
              {
              mm->SetCommand(command.command);
              if(command.command==8||command.command==7||command.command==1)mm->SetEnd(command.x,command.y);
              if(command.command==2)mm->SetTarget(place[command.x][command.y]);
              }
        }
